...
Citizen.CreateThread(function()
	...
	TriggerServerEvent('szi_communitycervice:checkIfSentenced')
end)
...
RegisterNetEvent('szi_communitycervice:inCommunityService')
AddEventHandler('szi_communitycervice:inCommunityService', function(actions_remaining)
	...
	while actionsRemaining > 0 and communityServiceFinished ~= true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.ServiceLocation.x, Config.ServiceLocation.y, Config.ServiceLocation.z) > 45 then
			ESX.Game.Teleport(playerPed, Config.ServiceLocation)
				...
				TriggerServerEvent('szi_communitycervice:extendService')
				actionsRemaining = actionsRemaining + Config.ServiceExtensionOnEscape
		end

	end

	TriggerServerEvent('szi_communitycervice:finishCommunityService', -1)
...
RegisterNetEvent('szi_communitycervice:finishCommunityService')
AddEventHandler('szi_communitycervice:finishCommunityService', function(source)
			...
			for i = 1, #availableActions do
				...
				if distance < 1.5 then
					...
					if(IsControlJustReleased(1, 38))then
						...
						TriggerServerEvent('szi_communitycervice:completeService')
